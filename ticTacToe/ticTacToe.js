...
//   });
  ...
  Game.prototype.placeMark = function (pos) {
    ...
    root.$('#' + pos[0] + "-" + pos[1]).css('background-color', Game.colors[this.player]);
  };
...
//   };
  ...
  Game.prototype.clickHandlers = function() {
    ...
    $('#board').on('click', '.square', function(event) {
      var square = $(event.target).attr('id');
      square = square.split('-');
      game.move(square);
      if (game.winner()) {
        var winner = game.winner();
        root.alert(Game.colors[winner]+ " has won!");
        $('#board').off();
      }
    })
  }
